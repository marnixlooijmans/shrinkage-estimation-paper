...
def run_parameter_inference(mat_type, est_type, nmocks, start, end, ncpus):
    ...
    for v in range(start, end+1):
        ...
        start = np.empty((nwalkers, ndim))
        start[:,0] = np.random.uniform(0.6, 3.4, nwalkers)
        start[:,1] = np.random.uniform(0.1, 1.9, nwalkers)

        with Pool() as pool:
            sampler = emcee.EnsembleSampler(nwalkers, ndim, powerlike.log_posterior, pool=pool)
            sampler.run_mcmc(start, nsteps, progress=True)
        
        results = sampler.get_chain()
        np.save(f"../output/{mat_type}_{est_type}_{nmocks}_results_emcee_v{v}", results)
